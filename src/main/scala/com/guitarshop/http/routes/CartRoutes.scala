...
import com.guitarshop.http.auth.users.CommonUser
import com.guitarshop.http.vars.ItemIdVar
  ...
  private val httpRoutes: AuthedRoutes[CommonUser, F] = AuthedRoutes.of {
    ...
    case ar @ POST -> Root as user =>
      ar.req.asJsonDecode[Cart].flatMap {
        _.items
          .map { case (id, quantity) => shoppingCart.add(user.value.id, id, quantity) }
          .toList
          .sequence *> Created()
      }
