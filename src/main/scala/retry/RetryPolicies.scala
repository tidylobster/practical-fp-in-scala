...
object RetryPolicies {
  ...
  /** Delay(n) = Delay(n - 2) + Delay(n - 1)
    *
    * e.g. if `baseDelay` is 10 milliseconds, the delays before each retry will be 10 ms, 10 ms, 20
    * ms, 30ms, 50ms, 80ms, 130ms, ...
    */
  ...
  /** Add an upper bound to a policy such that once the given time-delay amount <b>per try</b> has
    * been reached or exceeded, the policy will stop retrying and give up. If you need to stop
    * retrying once <b>cumulative</b> delay reaches a time-delay amount, use
  ...
  /** Add an upperbound to a policy such that once the cumulative delay over all retries has reached
    * or exceeded the given limit, the policy will stop retrying and give up.
    */
